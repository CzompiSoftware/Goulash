# Run Goulash on Google Cloud Platform
...
* Create a database named `goulash`. This can be done through the Google Cloud Console. Use `utf8mb4` as the Character set.
...
```json
{
    ...
    "Database": {
        "Type": "MySql",
        "ConnectionString": "Server=YOURIP;User Id=root;Password=***;Database=goulash;CertificateFile=C:\\Path\\To\\client.pfx;CACertificateFile=C:\\Path\\To\\server-ca.pem;SSL Mode=VerifyCA"
    },
    ...
}
```
...
* `DBNAME` -- the name of the Goulash database on that instance (e.g., `goulash` in the instructions above)
